...
- `!rmlist`: Lists your upcoming reminders across all servers and lets you delete one by replying with its number. If used in a server channel, the list is sent to you via DM and the original request is acknowledged with a ✅ reaction. 📋
...
- `/rm {time} {reminder content}`: Sends a reminder to the channel at the specified time. ⏰
- `/rmdm {time} {reminder content}`: Sends a personal DM reminder at the specified time. ⏰
- `/rmlist`: Lists your upcoming reminders across all servers and lets you delete one by replying with its number. If used in a server channel, the list is sent to you via DM and you’ll receive an ephemeral confirmation. 📋
- `/note fetchall`: Downloads a zip archive of all server notes as CSV. 📦
- `/notehelp`: Shows this help message for note commands. ❓
